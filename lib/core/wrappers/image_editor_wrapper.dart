...
class ImageEditorWrapper {
  ...
  /// Flip image
  Future<Uint8List?> flipImage(Uint8List imageBytes,
      {bool horizontal = true, bool vertical = false}) async {
    final ImageEditorOption option = ImageEditorOption();
  ...
  Future<Uint8List?> addText(Uint8List imageBytes, String text,
      {int fontSize = 24, int color = 0xFFFFFFFF}) async {
  ...
  Future<Uint8List?> editImage(
      Uint8List imageBytes, ImageEditorOption option) async {
    return await ImageEditor.editImage(
        image: imageBytes, imageEditorOption: option);
  }
}
