...
class RizikEdgeTTSClient {
  ...
  final StreamController<void> _turnEndController =
      StreamController.broadcast();
  ...
  Stream<void> get turnEndStream =>
      _turnEndController.stream; // Signal when TTS finishes a sentence

  /// 1. Initialize by fetching Remote Config (with Sec-MS-GEC)
  Future<void> init() async {
    try {
      final response =
          await http.get(Uri.parse('${EnvConfig.backendUrl}/api/voice/config'));
      if (response.statusCode == 200) {
        ...
        print(
            "✅ TTS Config Loaded: $_voice (Sec-MS-GEC: ${_secMsGec?.substring(0, 8)}...)");
      } else {
  ...
  Future<void> synthesize(String text) async {
    ...
    print(
        "📤 Sent SSML request for: ${text.substring(0, text.length > 30 ? 30 : text.length)}...");
  }
