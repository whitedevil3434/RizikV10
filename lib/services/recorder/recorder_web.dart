...
class _RecorderWeb implements UniversalRecorder {
  ...
  Future<void> start(StreamSink<Uint8List> sink) async {
    try {
      ...
      _processor!.connect(
          _audioContext!.destination); // Needed for Chrome to fire events
      ...
      _processor!.onaudioprocess = (web.AudioProcessingEvent event) {
        ...
        if (!hasLogged) {
          print(
              "WEB: Audio Process Fired! Input Samples: ${inputData.length} (Amp: $maxAmp)");
          hasLogged = true;
        }
  ...
  Uint8List _resampleAndConvert(
      Float32List input, double inRate, double outRate) {
    if (inRate == outRate) {
