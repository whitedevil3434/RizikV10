...
console.log('Starting Supabase MCP server to check columns...');
...
function handleMessage(msg) {
    if (msg.id === 1 && step === 0) {
        ...
        if (msg.error) {
            console.error('❌ Failed to check columns:', msg.error);
            process.exit(1);
        }
        console.log('✅ Column check result:', JSON.stringify(msg.result));
        process.exit(0);
    }
}
...
send({
    ...
    params: {
        ...
        capabilities: {},
        clientInfo: { name: 'check-client', version: '1.0.0' }
    }
});
