...
// ─────────────────────────────────────────────────────────────
// 2. SPATIAL ORGANISM (Treemap Engine)
// ─────────────────────────────────────────────────────────────
type Rect = { x: number, y: number, w: number, h: number };

// A simple slice-and-dice treemap algorithm for beautiful rectangular spatial allocation
...
function WorkField({ nodes, width, height, onNodeTap, expandedNodeId, isRoot = true }: { nodes: Record<string, CausalNode>; width: number; height: number; onNodeTap: (id: string | null) => void; expandedNodeId: string | null; isRoot?: boolean }) {
    ...
    const rects = useMemo(() => generateTreemap(weights, width, height, 12), [weights, width, height]);
    ...
    const getColors = (node: CausalNode, isExpanded: boolean) => {
        ...
        if (node.state === "ghost") return { bg: "#f1f5f9", border: "1px dashed #cbd5e1", text: "#94a3b8" };
        if (node.type === "Source") return { bg: "#ffffff", border: "none", text: "#334155" };
        if (node.type === "Need") return { bg: "#e0f2fe", border: "none", text: "#0284c7" };
        if (node.type === "Gap" || node.state === "blocked") return { bg: "#fee2e2", border: "none", text: "#dc2626" };
        if (node.type === "Capability") return { bg: "#dcfce7", border: "none", text: "#16a34a" };
        if (node.type === "Actor") return { bg: "#f3e8ff", border: "none", text: "#9333ea" };
        return { bg: "#ffffff", border: "none", text: "#0f172a" };
    };
