...
exports.fillerPolicy = exports.applyBladerHumanizerPolicy = void 0;
exports.calculateMaxAllowedFillers = calculateMaxAllowedFillers;
exports.getQualityMetrics = getQualityMetrics;
exports.applyHumanizerQualityPolicy = applyHumanizerQualityPolicy;
exports.applyStructureRefinement = applyStructureRefinement;
exports.transformText = transformText;
const fidelityEngine_1 = require("./fidelityEngine");
const chaosEngine_1 = require("./chaosEngine");
const qualityEngine_1 = require("./qualityEngine");
const humanErrorEngine_1 = require("./humanErrorEngine");
const bladerHumanizer_1 = require("./bladerHumanizer");
var bladerHumanizer_2 = require("./bladerHumanizer");
Object.defineProperty(exports, "applyBladerHumanizerPolicy", { enumerable: true, get: function () { return bladerHumanizer_2.applyBladerHumanizerPolicy; } });
...
const allLeadFillers = [
    ...exports.fillerPolicy.allowedNeutralFillers,
    ...exports.fillerPolicy.nonAllowedFillerPhrases,
...
function calculateMaxAllowedFillers(wordCount) {
    if (wordCount >= 100) {
...
function replaceForbiddenShortForms(text) {
    let next = text;
    for (const [token, replacement] of Object.entries(exports.fillerPolicy.forbiddenReplacements)) {
...
function getQualityMetrics(text) {
    const normalized = (text || "").trim();
    const wordCount = countWords(normalized);
    const fillerCount = exports.fillerPolicy.allowedNeutralFillers.reduce((sum, phrase) => {
        const regex = new RegExp(`\\b${escapeRegExp(phrase)}\\b`, "gi");
        return sum + (normalized.match(regex) || []).length;
    }, 0);
    const forbiddenCount = Object.keys(exports.fillerPolicy.forbiddenReplacements).reduce((sum, token) => {
        const regex = new RegExp(`\\b${escapeRegExp(token)}\\b`, "gi");
        return sum + (normalized.match(regex) || []).length;
    }, 0);
...
function applyHumanizerQualityPolicy(text) {
    ...
    const cleanedSentences = sentences.map((sentence) => {
        ...
        if (!lead)
            return next;
        const isAllowed = exports.fillerPolicy.allowedNeutralFillers.includes(lead);
        if (!isAllowed) {
    ...
    const finalSentences = output
        ...
        if (!lead)
            return sentence;
        if (!exports.fillerPolicy.allowedNeutralFillers.includes(lead)) {
...
function stripLeadDiscourse(sentence) {
    const prefixes = exports.fillerPolicy.allowedNeutralFillers.sort((a, b) => b.length - a.length);
...
function applyStructureRefinement(text, mode) {
    if (!text.trim())
        return text;
...
async function transformText(aiText, dnaProfile, env, options = {}) {
    ...
    try {
        for (const block of blocks) {
            ...
            factAtoms.forEach((atom, idx) => {
                ...
                targetContent = fidelityEngine_1.FidelityEngine.aiClichePurge(targetContent);
                if (!targetContent)
                ...
                if (isAssignmentMode) {
                    ...
                    if (chosenConnector) {
                        ...
                        if (recentAssignmentConnectors.length > 5)
            ...
            outputParagraph = fidelityEngine_1.FidelityEngine.alignPhrasing(outputParagraph, isAcademic, fidelitySeed);
            if (isAcademic) {
                outputParagraph = fidelityEngine_1.FidelityEngine.purgeSlang(outputParagraph);
            ...
            const qualityCheck = isAssignmentMode
                ...
                : (0, qualityEngine_1.evaluateHumanQuality)(outputParagraph, paragraphText);
            if (!isAssignmentMode && (0, qualityEngine_1.shouldRegenerateByQuality)(qualityCheck)) {
                // One retry with shifted seed
                const retryAssembly = outputParagraph; // save current
                let retryOutput = (0, chaosEngine_1.applyStructuralChaos)(outputParagraph, userDnaSeed + '_quality_retry', isAcademic);
                retryOutput = applyHumanizerQualityPolicy(retryOutput);
                if (useBladerHumanizer)
                    retryOutput = (0, bladerHumanizer_1.applyBladerHumanizerPolicy)(retryOutput);
                const retryQuality = (0, qualityEngine_1.evaluateHumanQuality)(retryOutput, paragraphText);
            ...
            if (!isAssignmentMode) {
                while (passCount < 2 && !entropySufficient) {
                    passCount++;
                    finalParagraphOutput = (0, chaosEngine_1.applyStructuralChaos)(finalParagraphOutput, userHash + passCount.toString(), isAcademic);
            ...
            if (!isAssignmentMode) {
                ...
                if (useBladerHumanizer)
                    outputParagraph = (0, bladerHumanizer_1.applyBladerHumanizerPolicy)(outputParagraph);
            ...
            if (env.AI && !isAssignmentMode) {
                ...
                try {
                    const sysPrompt = humanErrorThreshold > 0 ? (0, humanErrorEngine_1.PRESERVE_ERRORS_SYSTEM_PROMPT)(isAcademic) : `You are a GOD-MODE sentence restructuring engine. TOTAL STRUCTURAL INVERSION.`;
                ...
                if (humanErrorThreshold > 0) {
                    ...
                    const reInjectResult = (0, humanErrorEngine_1.lightReInjectErrors)(llmPara, safetyConfig);
                ...
                if (useBladerHumanizer)
                    llmPara = (0, bladerHumanizer_1.applyBladerHumanizerPolicy)(llmPara);
