...
export async function extractDNA(text: string, env: any) {
  ...
  sentenceStarts.forEach(w => { if(w) startFreq[w] = (startFreq[w] || 0) + 1; });
  ...
  try {
    ...
    const aiResponse: any = await Promise.race([embeddingPromise, timeoutPromise]);
  ...
  return {
    ...
    errorFingerprint: {
        ...
        error_factor: Math.min(2.0, Math.max(0.3, 
