...
const ARTICLE_SWAPS: Record<string, string[]> = {
  "a": ["the", "an"],
  "an": ["the", "a"],
  "the": ["a", "an"],
};
...
const CONFUSED_WORDS: Record<string, string> = {
  "affect": "effect",
  "effect": "affect",
  "its": "it's",
  "it's": "its",
...
const L1_INTERFERENCE: Record<string, string> = {
...
const WRONG_COLLOCATIONS: Record<string, string> = {
...
const DOUBLE_COMPARATIVES: Record<string, string> = {
  "better": "more better",
  "worse": "more worse",
  "best": "most best",
  "worst": "most worst",
...
const AM_BR_SPELLING: Record<string, string> = {
  "analyze": "analyse",
  "color": "colour",
  "favor": "favour",
  "honor": "honour",
...
const VERB_TENSE_MAP: Record<string, Record<string, string>> = {
...
const ALL_VERB_FORMS: Map<string, { base: string, form: string }> = new Map();
...
const PRONOUN_CASE_SWAPS: Record<string, string> = {
  "i": "me",
  "he": "him",
  "she": "her",
  "we": "us",
...
const QUANTIFIER_SWAPS: Record<string, string> = {
  "fewer": "less",
  "less": "fewer",
  "many": "much",
  "much": "many",
...
const TRANSITION_SWAPS: Record<string, string[]> = {
...
const CAUSAL_SWAPS: Record<string, string[]> = {
...
function applyVerbTenseChaos(
  ...
  for (const [verbForm, info] of ALL_VERB_FORMS) {
    if (verbForm.length < 3) continue; // Skip tiny words like "go", "do" etc to avoid false positives
  ...
  output = output.replace(reportedSpeechPattern, (match, verb, that, pronoun, aux) => {
    s = simpleHash(s.toString() + match);
    if (seededRandom(s) > prob * 0.7) return match;

    const presentAux: Record<string, string> = {
  ...
  output = output.replace(ingEdPattern, (match, stem, suffix) => {
    if (flipCount >= 3) return match; // Cap at 3 flips per text
    s = simpleHash(s.toString() + match);
    if (seededRandom(s) > prob * 0.3) return match;
...
function applyArticleErrors(
  ...
  return text.replace(/\b(a|an|the)\b/gi, (match) => {
    if (changeCount >= maxChanges) return match;
...
function applyPrepositionErrors(
  text: string, prob: number, seed: number, log: ErrorMutationLog[]
): string {
  let s = seed;
  let changeCount = 0;
...
function applyNounPluralErrors(
  text: string, prob: number, seed: number, log: ErrorMutationLog[]
): string {
  let s = seed;
  let changeCount = 0;
  ...
  return text.replace(/\b(\w+)\b/g, (match) => {
    if (changeCount >= 4) return match;
    const lower = match.toLowerCase();
    if (!UNCOUNTABLE_NOUNS.has(lower)) return match;
...
function applyDoubleComparatives(
  text: string, prob: number, seed: number, log: ErrorMutationLog[]
): string {
  let s = seed;
  let output = text;
...
function applyQuantifierErrors(
  text: string, prob: number, seed: number, log: ErrorMutationLog[]
): string {
  let s = seed;
  let output = text;
...
function applyCapitalizationErrors(
  text: string, prob: number, seed: number, log: ErrorMutationLog[]
): string {
  let s = seed;
  let changeCount = 0;
  ...
  return text.replace(/\b([A-Z][a-z]{3,})\b/g, (match, word, offset) => {
    ...
    if (offset === 0 || /[.!?]\s*$/.test(text.substring(0, offset))) return match;
...
function applyWrongCollocations(
  text: string, prob: number, seed: number, log: ErrorMutationLog[]
): string {
  let s = seed;
  let output = text;
 ...
 */
export function lightReInjectErrors(
  text: string,
  config: HumanErrorConfig
...
export const PRESERVE_ERRORS_SYSTEM_PROMPT = (isAcademic: boolean) => `You are an academic polisher. Your ONLY job is to improve natural flow and add reflective scholarly depth.
