...
export type MicroStyle = "pure-formal" | "transitional-formal" | "conversational-formal";
export type ClauseType = "main" | "subordinate" | "contrast";
...
const COLLOCATION_MATRIX: Record<string, string[]> = {
...
function mixVocabularyAndGrammar(
    sentence: string, 
    seedBase: number, 
    isAcademic: boolean,
    ...
    for (let i = 0; i < words.length; i++) {
        const word = words[i];
        if (!word.trim() || !/[a-zA-Z]/.test(word)) continue;
    ...
    if (r < 0.05 && isAcademic) {
        ...
        modified = modified.replace(/\b(it is)\b/ig, (match) => 
            match.charAt(0) === "I" ? "It's" : "it's"
        );
        modified = modified.replace(/\b(do not)\b/ig, (match) => 
            match.charAt(0) === "D" ? "Don't" : "don't"
        );
    }
 ...
 */
function heuristicClauseRegexSplit(sentence: string): string[] {
...
function labelClause(text: string): ClauseNode {
    ...
    let microStyle: MicroStyle = "pure-formal";
    ...
    if (isConversational) microStyle = "conversational-formal";
    else if (isTransitional) microStyle = "transitional-formal";
...
function blendClauses(clauses: ClauseNode[], seedBase: number, isAcademic: boolean): string {
...
export function applyStructuralChaos(text: string, userDnaHash: string, isAcademic: boolean): string {
    if (!text) return text;
    ...
    const shuffledOutput: string[] = [];
    
    let currentSeed = baseSeed;
    // Global tracking to eliminate AI repetition
    const globalWordUsage: Record<string, number> = {};
    ...
    for (let i = 0; i < sentences.length; i++) {
        ...
        if (currentType === lastStructureType) {
            if (currentType === "SHORT" && i < sentences.length - 1) {
                ...
                const connector = FORMAL_MARKERS.connectors.addition[
                    Math.floor(seededRandom(currentSeed + 3) * FORMAL_MARKERS.connectors.addition.length)
                ];
