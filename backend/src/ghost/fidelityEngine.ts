...
export class FidelityEngine {
  ...
  private static HUMAN_3GRAMS: Record<string, string[]> = {
  ...
  private static ACADEMIC_3GRAMS: Record<string, string[]> = {
   ...
   */
  static alignPhrasing(text: string, isAcademic: boolean = false, seed: number = 42): string {
    let aligned = text;
   ...
   */
  static purgeSlang(text: string): string {
    const slangBlacklist: Record<string, string> = {
      "lol": "",
      "tbh": "to be honest",
      "idk": "i do not know",
      "bruh": "",
   ...
   */
  static aiClichePurge(text: string): string {
    let purged = text;
    ...
    const aiVocabularyMap: Record<string, string> = {
      "tapestry": "mix",
      "delve": "look",
      "nuances": "details",
      "testament": "proof",
