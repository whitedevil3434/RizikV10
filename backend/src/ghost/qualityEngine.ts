export type HumanQualityMetrics = {
  wordCount: number;
...
function repeatedNgramRatio(tokens: string[]): number {
  ...
  for (let i = 0; i < tokens.length - 2; i += 1) {
    ...
    seen.set(key, next);
    if (next > 1) repeated += 1;
  }
  return total ? repeated / total : 0;
}

export function evaluateHumanQuality(output: string, sourceText = ""): HumanQualityScore {
  ...
  for (const sentence of normalizedSentences) {
    if (sentenceSet.has(sentence)) repeatedSentenceCount += 1;
  ...
  if (src) {
    ...
    if (srcTokens.size > 0) {
      ...
      for (const token of srcTokens) {
        if (outTokens.has(token)) overlap += 1;
      }
...
export function shouldRegenerateByQuality(score: HumanQualityScore): boolean {
