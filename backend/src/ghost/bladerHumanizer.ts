...
type CaseStyle = "lower" | "upper" | "capitalized" | "mixed";

function detectCaseStyle(word: string): CaseStyle {
  if (!word) return "lower";
  if (word.toUpperCase() === word && /[A-Z]/.test(word)) return "upper";
  const first = word[0] || "";
...
function applyCaseStyle(template: string, replacement: string): string {
  const style = detectCaseStyle(template);
  if (style === "upper") return replacement.toUpperCase();
  if (style === "capitalized") return replacement.charAt(0).toUpperCase() + replacement.slice(1);
  if (style === "lower") return replacement.toLowerCase();
  return replacement;
}
...
function stripCommonChatbotArtifacts(text: string): string {
  ...
  for (const line of lines) {
    ...
    if (bannedLine.test(line.trim())) continue;
    cleaned.push(line);
  }
...
function stripSignposting(text: string): string {
  let out = String(text || "");
  // Start-of-paragraph signposting.
  out = out.replace(
    /(^|\n\s*\n)\s*(let['’]s dive in|here['’]s what you need to know|below (?:is|are)|in this (?:section|article)|to summarize|in summary|in conclusion)\s*[:.\-—]*\s*/gi,
    "$1",
  );
...
function simplifyFillerPhrases(text: string): string {
  let out = String(text || "");
...
function simplifyCopulaAvoidance(text: string): string {
  let out = String(text || "");
...
export function applyBladerHumanizerPolicy(text: string): string {
  let out = String(text || "");
