...
function replaceAIVocabulary(text, seed) {
    let out = String(text || "");
    ...
    // Stochastic Additive Connector Rotation (Eliminates "Also" addiction)
    const additiveTriggers = ["additionally", "moreover", "furthermore"];
    for (const trigger of additiveTriggers) {
        const re = new RegExp(`\\b${trigger}\\b`, "gi");
        out = out.replace(re, (m) => {
            const rand = Math.abs((Math.sin(seed + out.length) * 10000) % 1);
            if (rand < 0.25)
                return applyCaseStyle(m, "also");
            if (rand < 0.40)
                return applyCaseStyle(m, "too"); // Note: "too" at start might be weird, but we handle in clean-up
            if (rand < 0.55 && text.length > 200)
                return m; // Keep original if it's a long text
            return ""; // Purge entirely to force sentence flow
        });
    }
