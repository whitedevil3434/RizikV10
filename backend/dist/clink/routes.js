...
clinkApp.use("/api/clink/*", async (c, next) => {
    ...
    if (!isPublicShareRoute) {
        const userId = await verifyClinkBearer(c.req.raw, c.env.SUPABASE_JWT_SECRET);
        if (!userId) {
            c.header("WWW-Authenticate", "Bearer");
            return c.json({ error: "CLINK_AUTH_REQUIRED", message: "Sign in to access this workspace" }, 401);
        }
    ...
    if (c.req.method !== "GET" && idempotencyKey) {
        ...
        if (previous)
            return c.json(previous.body, previous.status);
    }
    ...
    if (ifMatch && ifMatch !== "*" && commitmentMatch) {
        ...
        if (current && (!Number.isInteger(expectedVersion) || expectedVersion !== current.aggregateVersion))
            return c.json({ error: "CLINK_CONFLICT", message: "This record changed since it was loaded", currentVersion: current.aggregateVersion }, 409);
    }
    await next();
    if (c.req.method !== "GET" && c.res.status < 400) {
        if (idempotencyKey) {
            ...
            if (body)
                clinkStore.setIdempotency(idempotencyKey, requestOwner, c.res.status, body);
        }
        await clinkStore.persist(env.RIZIK_STORAGE);
    }
});
function ownerId(c) { return c.get("clinkUserId"); }
function owned(c, id) { return clinkStore.assertOwner(id, ownerId(c)); }
function jsonError(error) {
    if (error instanceof ClinkDomainError)
...
async function buildFirebaseShadowPassword(firebaseUid, salt) {
...
async function syncFirebaseUser(c, idToken) {
    const env = c.env;
    if (!env.FIREBASE_API_KEY || !env.NEXT_PUBLIC_SUPABASE_URL || !env.SUPABASE_SERVICE_ROLE_KEY)
        throw new ClinkDomainError("GOOGLE_AUTH_UNAVAILABLE", "Google sign-in is not configured", 503);
    ...
    if (!lookup.ok || !firebaseUser?.localId || !firebaseUser.email)
        throw new ClinkDomainError("GOOGLE_AUTH_INVALID", "Google sign-in could not be verified", 401);
    ...
    if (!userId)
        throw new ClinkDomainError("GOOGLE_USER_SYNC_FAILED", "Google account could not be linked to a Rizik account", 502);
    ...
    if (!sessionResponse.ok || !session.access_token || !session.refresh_token)
        throw new ClinkDomainError("GOOGLE_SESSION_FAILED", "Google account was linked but a workspace session could not be created", 502);
...
clinkApp.post("/api/clink/v1/capabilities", async (c) => {
    try {
        return c.json({ data: clinkStore.createCapabilityNode({ ...(await c.req.json()), ownerUserId: ownerId(c) }) }, 201);
    }
    catch (error) {
...
clinkApp.post("/api/clink/v1/capabilities/:id/availability", async (c) => {
    try {
        ...
        return c.json({ data: clinkStore.updateCapabilityAvailability(c.req.param("id"), ownerId(c), body.status, body.availabilityNote) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/capabilities/:id/dependencies", async (c) => {
    try {
        const body = await c.req.json();
        return c.json({ data: clinkStore.addCapabilityDependency(c.req.param("id"), ownerId(c), body.dependsOnCapabilityId, body.relation || "requires", body.note) }, 201);
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/dependencies", async (c) => {
    try {
        return c.json({ data: clinkStore.createDependency({ ...(await c.req.json()), ownerUserId: ownerId(c) }) }, 201);
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/expectations", async (c) => {
    try {
        return c.json({ data: clinkStore.createExpectation({ ...(await c.req.json()), ownerUserId: ownerId(c) }) }, 201);
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.get("/api/clink/v1/expectations/:id", (c) => {
    try {
        return c.json({ data: safeExpectation(clinkStore.assertExpectationOwner(c.req.param("id"), ownerId(c))) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.get("/api/clink/v1/expectations/:id/timeline", (c) => {
    try {
        const expectation = clinkStore.assertExpectationOwner(c.req.param("id"), ownerId(c));
        return c.json({ data: { events: expectation.events, transitions: expectation.transitions } });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/expectations/:id/send", (c) => {
    try {
        clinkStore.assertExpectationOwner(c.req.param("id"), ownerId(c));
        return c.json({ data: clinkStore.sendExpectation(c.req.param("id")) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/expectations/:id/withdraw", (c) => {
    try {
        return c.json({ data: safeExpectation(clinkStore.withdrawExpectation(c.req.param("id"), ownerId(c))) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/expectations/:id/convert-to-commitment", (c) => {
    try {
        return c.json({ data: safeCommitment(clinkStore.convertExpectationToCommitment(c.req.param("id"), ownerId(c))) }, 201);
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/auth/firebase", async (c) => {
    try {
        const body = await c.req.json();
        if (!body.idToken)
            throw new ClinkDomainError("GOOGLE_TOKEN_REQUIRED", "Google sign-in token is required");
        return c.json({ data: await syncFirebaseUser(c, body.idToken) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/share/expectation/:token/respond", async (c) => {
    try {
        return c.json({ data: safeExpectation(clinkStore.respondExpectationByToken(c.req.param("token"), await c.req.json())) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.get("/api/clink/v1/commitments/:id/timeline", (c) => {
    try {
        const commitment = owned(c, c.req.param("id"));
        return c.json({ data: { events: commitment.events, transitions: commitment.transitions } });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.get("/api/clink/v1/commitments/:id/capabilities", (c) => {
    try {
        const commitment = owned(c, c.req.param("id"));
        return c.json({ data: { requirements: clinkStore.listCapabilities(commitment.id), assignments: clinkStore.listAssignments(commitment.id) } });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/commitments/:id/capabilities", async (c) => {
    try {
        const commitment = owned(c, c.req.param("id"));
        const body = await c.req.json();
        return c.json({ data: clinkStore.addRequiredCapability(commitment.id, body.createdBy || ownerId(c), body, ownerId(c)) }, 201);
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/commitments/:id/assignments", async (c) => {
    try {
        const commitment = owned(c, c.req.param("id"));
        const body = await c.req.json();
        return c.json({ data: clinkStore.assignCapability(commitment.id, body.assignedBy || ownerId(c), body) }, 201);
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/commitments/:id/assignments/:assignmentId/block", async (c) => {
    try {
        ...
        return c.json({ data: clinkStore.blockAssignment(commitment.id, c.req.param("assignmentId"), ownerId(c), body.note) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/share/:token/accept", (c) => {
    try {
        const item = clinkStore.getByShareToken(c.req.param("token"));
        if (!item)
            return c.json({ error: "LINK_EXPIRED_OR_REVOKED" }, 410);
        return c.json({ data: safeCommitment(clinkStore.accept(item.id, "counterparty")) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/share/:token/reject", async (c) => {
    try {
        ...
        if (!item)
            return c.json({ error: "LINK_EXPIRED_OR_REVOKED" }, 410);
        const body = await c.req.json().catch(() => ({}));
        return c.json({ data: safeCommitment(clinkStore.reject(item.id, "counterparty", body.reason)) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/share/:token/change-request", async (c) => {
    try {
        ...
        if (!item)
            return c.json({ error: "LINK_EXPIRED_OR_REVOKED" }, 410);
        const body = await c.req.json().catch(() => ({}));
        return c.json({ data: safeCommitment(clinkStore.requestChange(item.id, "counterparty", body.reason || "Terms need clarification")) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/share/:token/evidence/:evidenceId/download-token", (c) => {
    try {
        ...
        if (!item)
            return c.json({ error: "LINK_EXPIRED_OR_REVOKED" }, 410);
        const evidence = clinkStore.listEvidence(item.id).find((entry) => entry.id === c.req.param("evidenceId"));
        if (!evidence || evidence.visibility !== "shared")
            return c.json({ error: "EVIDENCE_NOT_AVAILABLE" }, 404);
        return c.json({ data: clinkStore.createEvidenceDownloadToken(item.id, evidence.id) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/commitments", async (c) => {
    try {
        return c.json({ data: clinkStore.create({ ...(await c.req.json()), ownerUserId: ownerId(c) }) }, 201);
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/commitments/:id/send", (c) => {
    try {
        ...
        const result = clinkStore.createShare(c.req.param("id"));
...
clinkApp.post("/api/clink/v1/commitments/:id/accept", async (c) => {
    try {
        owned(c, c.req.param("id"));
        return c.json({ data: clinkStore.accept(c.req.param("id"), ownerId(c)) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/commitments/:id/start", async (c) => {
    try {
        const body = await c.req.json().catch(() => ({}));
        owned(c, c.req.param("id"));
        return c.json({ data: clinkStore.start(c.req.param("id"), ownerId(c), body) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/commitments/:id/reject", async (c) => {
    try {
        const body = await c.req.json().catch(() => ({}));
        owned(c, c.req.param("id"));
        return c.json({ data: clinkStore.reject(c.req.param("id"), ownerId(c), body.reason) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/commitments/:id/change-request", async (c) => {
    try {
        const body = await c.req.json();
        owned(c, c.req.param("id"));
        return c.json({ data: clinkStore.requestChange(c.req.param("id"), ownerId(c), body.reason || "Terms need clarification") });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/commitments/:id/amend", async (c) => {
    try {
        const body = await c.req.json();
        owned(c, c.req.param("id"));
        return c.json({ data: clinkStore.amend(c.req.param("id"), ownerId(c), body) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/commitments/:id/fulfill", async (c) => {
    try {
        const body = await c.req.json().catch(() => ({}));
        owned(c, c.req.param("id"));
        return c.json({ data: clinkStore.fulfill(c.req.param("id"), ownerId(c), body) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/commitments/:id/confirm", async (c) => {
    try {
        const body = await c.req.json().catch(() => ({}));
        owned(c, c.req.param("id"));
        return c.json({ data: clinkStore.confirm(c.req.param("id"), ownerId(c), body.accepted !== false, body) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/commitments/:id/evidence", async (c) => {
    try {
        const body = await c.req.json();
        owned(c, c.req.param("id"));
        return c.json({ data: clinkStore.addEvidence(c.req.param("id"), ownerId(c), body) }, 201);
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/commitments/:id/evidence/upload", async (c) => {
    try {
        ...
        if (!fileEntry || typeof fileEntry !== "object" || !("arrayBuffer" in fileEntry))
            throw new ClinkDomainError("EVIDENCE_FILE_REQUIRED", "An evidence file is required");
        owned(c, c.req.param("id"));
        const file = fileEntry;
        if (file.size > 10 * 1024 * 1024)
            throw new ClinkDomainError("EVIDENCE_FILE_TOO_LARGE", "Evidence files must be 10 MB or smaller");
        const allowed = ["image/jpeg", "image/png", "image/webp", "application/pdf"];
        if (!allowed.includes(file.type))
            throw new ClinkDomainError("EVIDENCE_FILE_TYPE_NOT_ALLOWED", "Only JPG, PNG, WEBP and PDF evidence files are supported");
        const bucket = c.env.RIZIK_STORAGE;
        if (!bucket)
            throw new ClinkDomainError("EVIDENCE_STORAGE_UNAVAILABLE", "Evidence storage is unavailable", 503);
...
clinkApp.get("/api/clink/v1/commitments/:id/evidence", (c) => {
    try {
        owned(c, c.req.param("id"));
        return c.json({ data: clinkStore.listEvidence(c.req.param("id")) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/commitments/:id/evidence/:evidenceId/download-token", (c) => {
    try {
        owned(c, c.req.param("id"));
        return c.json({ data: clinkStore.createEvidenceDownloadToken(c.req.param("id"), c.req.param("evidenceId")) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/commitments/:id/disputes", async (c) => {
    try {
        const body = await c.req.json();
        owned(c, c.req.param("id"));
        return c.json({ data: clinkStore.openDispute(c.req.param("id"), ownerId(c), body.issueType || "other", body.claim || {}) }, 201);
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/disputes/:id/respond", async (c) => {
    try {
        const body = await c.req.json();
        const dispute = clinkStore.getDispute(c.req.param("id"));
        if (!dispute)
            return c.json({ error: "NOT_FOUND" }, 404);
        owned(c, dispute.commitmentId);
        return c.json({ data: clinkStore.respondDispute(c.req.param("id"), ownerId(c), body.message || "") });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/disputes/:id/resolve", async (c) => {
    try {
        ...
        if (!dispute)
            return c.json({ error: "NOT_FOUND" }, 404);
        owned(c, dispute.commitmentId);
        return c.json({ data: clinkStore.resolveDispute(c.req.param("id"), ownerId(c), body.resolution || {}) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/commitments/:id/settlement", async (c) => {
    try {
        const body = await c.req.json();
        owned(c, c.req.param("id"));
        return c.json({ data: clinkStore.recordSettlement(c.req.param("id"), body, ownerId(c)) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/commitments/:id/revoke-share", (c) => {
    try {
        owned(c, c.req.param("id"));
        return c.json({ data: clinkStore.revokeShare(c.req.param("id")) });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.post("/api/clink/v1/commitments/:id/close", async (c) => {
    try {
        const body = await c.req.json();
        owned(c, c.req.param("id"));
        return c.json({ data: clinkStore.close(c.req.param("id"), ownerId(c), body.reason || "completed") });
    }
    catch (error) {
        const result = jsonError(error);
        return c.json(result.body, result.status);
...
clinkApp.get("/api/clink/v1/evidence/download/:token", async (c) => {
    ...
    if (!record?.evidence?.objectKey)
        return c.json({ error: "EVIDENCE_LINK_EXPIRED" }, 410);
    const bucket = c.env.RIZIK_STORAGE;
    const object = bucket ? await bucket.get(record.evidence.objectKey) : null;
    if (!object)
        return c.json({ error: "EVIDENCE_NOT_FOUND" }, 404);
