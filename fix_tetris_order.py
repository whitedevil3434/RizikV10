...
function calculateWeights(nodes: Record<string, CausalNode>, expandedId: string | null): Record<string, number> {
  ...
  for (const [id, node] of Object.entries(nodes)) {
    if (expandedId === id) weights[id] = 40.0; // Expanded gets more space
    else if (expandedId) {
...
// ─────────────────────────────────────────────────────────────
// 2. TERRITORY SOLVER (Numerical Discretization / TETRIS)
// ─────────────────────────────────────────────────────────────
const COLS = 16;
const ROWS = 12;
...
function allocateTerritories(weights: Record<string, number>) {
  ...
  // FIX: DO NOT SORT BY WEIGHT. Sort by FIXED TOPOLOGY so they don't jump around when clicked!
  ...
  // BFS Growth (Creates Tetris / L-Shapes)
  let progress = true;
  while (progress) {
    progress = false;
