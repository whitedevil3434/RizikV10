...
// ─────────────────────────────────────────────────────────────
// 4. ONE WORK FIELD (Crisp Tetris Engine)
// ─────────────────────────────────────────────────────────────
...
function WorkField({ nodes, width, height, onNodeTap, expandedNodeId, isRoot = true, cols, rows }: { nodes: Record<string, CausalNode>; width: number; height: number; onNodeTap: (id: string | null) => void; expandedNodeId: string | null; isRoot?: boolean; cols: number; rows: number }) {
    ...
    const getColors = (node: CausalNode, isExpanded: boolean) => {
        ...
        else if (node.type === "Need") { fill = "#bae6fd"; text = "#0369a1"; } // Sky blue
    ...
    return (
        <div className="work-field" style={{ width, height, position: 'relative' }}>
            {/* CRISP SVG TETRIS GEOMETRY (NO GOO) */}
                                    ...
                                    if (cellId === id) {
                                        return (
                                            <motion.rect 
                                                key={`${x}-${y}`} 
                                                initial={false}
                                                animate={{ x: x*cellW, y: y*cellH, width: cellW+1, height: cellH+1, fill }}
                                                transition={SPRING}
                                                rx={8} /* Slight inner rounding for each cell to soften it just a bit, but still crisp */
                                            />
                                        );
                                    }
                                    return null;
            ...
            {/* HTML READABLE CONTENT (APPLE CARD STYLE) */}
                            ...
                            >
                                <div className="semantic-content" style={{ padding: isExpanded ? '24px' : '16px' }}>
                                    <div className="node-header">
                                        ...
                                        <div className="badge" style={{ color: text, background: 'rgba(255,255,255,0.4)', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
...
new_css = """
:root {
  --surface-default: #1e293b; /* Dark slate background */
  --color-primary: #3b82f6;
  --color-text-main: #f8fafc;
}
* { box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, "SF Pro Display", "Segoe UI", Roboto, sans-serif; }
body { margin: 0; background: var(--surface-default); color: var(--color-text-main); overflow: hidden; overscroll-behavior: none; }
...
.svg-territory.state-ghost { opacity: 0.9; }

/* The Rivers (Seams) cutting through */
.seam-normal { stroke: #1e293b; stroke-width: 12px; } 

/* CONTENT (APPLE CARD STYLE TEXT LAYER) */
...
.recursive-container { width: 100%; flex: 1; position: relative; margin-top: 12px; background: rgba(255,255,255,0.3); border-radius: 16px; border: 1px solid rgba(255,255,255,0.5); padding: 8px; display: flex; justify-content: center; align-items: center; }

/* Composer Bar */
.composer-bar { padding: 20px; display: flex; flex-direction: column; align-items: center; gap: 12px; position: absolute; bottom: 0; width: 100%; z-index: 100; pointer-events: none; background: linear-gradient(to top, rgba(30, 41, 59, 1) 40%, transparent); }
.onboarding-text { font-size: 14px; color: #94a3b8; font-weight: 600; letter-spacing: 0.5px; transition: opacity 0.3s ease; }
.composer-form { pointer-events: auto; display: flex; gap: 10px; width: 100%; max-width: 680px; box-shadow: 0 8px 32px rgba(0,0,0,0.5); border-radius: 30px; background: white; padding: 6px; border: 1px solid rgba(0,0,0,0.1); }
...
.composer-form button { width: 50px; height: 50px; border-radius: 25px; background: #0f172a; color: white; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: transform 0.2s; flex-shrink: 0; }
.composer-form button:hover { transform: scale(1.05); }
.composer-form button:disabled { background: #94a3b8; cursor: not-allowed; transform: none; }
